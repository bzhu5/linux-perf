				  ...
				  struct perf_sample *sample)
{
	...
	intel_bts_dump(bts, sample->itrace_sample.data,
				     ...
				     struct perf_tool *tool)
{
	...
	if (!branch->from) {
		if (branch->to)
			btsq->sample_flags = PERF_FLAG_BRANCH |
					     PERF_FLAG_TRACE_BEGIN;
		else
			btsq->sample_flags = 0;
		btsq->intel_pt_insn.length = 0;
	} else if (!branch->to) {
		btsq->sample_flags = PERF_FLAG_BRANCH | PERF_FLAG_TRACE_END;
		btsq->intel_pt_insn.length = 0;
	} else {
		...
		if (!machine__kernel_ip(btsq->bts->machine, branch->from) &&
		    ...
		    btsq->sample_flags != (PERF_FLAG_BRANCH | PERF_FLAG_CALL |
				  ...
				  struct perf_session *session)
{
	...
	if (bts->synth_opts.calls)
		bts->branches_filter |= PERF_FLAG_CALL | PERF_FLAG_ASYNC |
					PERF_FLAG_TRACE_END;
	if (bts->synth_opts.returns)
		bts->branches_filter |= PERF_FLAG_RETURN |
					PERF_FLAG_TRACE_BEGIN;
...
static size_t intel_bts_sample_size(const char **str)
{
	char *endptr;
					  ...
					  const char *str)
{
	...
	opts->itrace_sample_size = str ? intel_bts_sample_size(&str) : 0;
