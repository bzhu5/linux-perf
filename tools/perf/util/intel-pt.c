...
static void intel_pt_sample_flags(struct intel_pt_queue *ptq)
{
	if (ptq->state->flags & INTEL_PT_ABORT_TX) {
		ptq->flags = PERF_FLAG_BRANCH | PERF_FLAG_TX_ABORT;
	} else if (ptq->state->flags & INTEL_PT_ASYNC) {
		if (ptq->state->to_ip)
			ptq->flags = PERF_FLAG_BRANCH | PERF_FLAG_CALL |
				PERF_FLAG_ASYNC | PERF_FLAG_INTERRUPT;
		else
			ptq->flags = PERF_FLAG_BRANCH | PERF_FLAG_TRACE_END;
		ptq->insn_len = 0;
	} else {
		if (ptq->state->from_ip)
			ptq->flags = intel_pt_insn_type(ptq->state->insn_op);
		else
			ptq->flags = PERF_FLAG_BRANCH | PERF_FLAG_TRACE_BEGIN;
		if (ptq->state->flags & INTEL_PT_IN_TX)
			ptq->flags |= PERF_FLAG_IN_TX;
...
static u64 intel_pt_switch_ip(struct machine *machine)
{
	struct map *map;
	struct symbol *sym;
	...
	sym = dso__first_symbol(map->dso, MAP__FUNCTION);
	while (sym) {
				 ...
				 struct perf_session *session)
{
	...
	if (pt->synth_opts.calls)
		pt->branches_filter |= PERF_FLAG_CALL | PERF_FLAG_ASYNC |
				       PERF_FLAG_TRACE_END;
	if (pt->synth_opts.returns)
		pt->branches_filter |= PERF_FLAG_RETURN | PERF_FLAG_TRACE_BEGIN;
